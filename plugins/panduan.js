...
`.trim()
    ...
    let { search, deskripsi, judul, index } = json.find(v => v.search == args[0])
        return conn.send3ButtonLoc(m.chat, await(await fetch(image)).buffer(), `No. ${index} *${judul}*

${deskripsi}

Query: ${search}
`.trim(), watermark, 'Owner', '.owner', 'Menu', '.?', 'Sewa', '.sewa ', m)
}
handler.tags = ['main']
...
global.panduan = [
    ...
    },
    {
        "index": "3",
        "search": "add",
        "deskripsi": "Silahkan baca pada menu sewa dengan mengetikkan *.sewa*. Haruno bot memberlakukan trial gratis 3 hari. Setelah 3 hari maka bot akan keluar dari group. ada opsi 1 minggu, 1 bulan dan 2 bulan berlangganan.",
